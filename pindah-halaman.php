...
if ($user_role === 'Mahasiswa') {
    if ($id_sub_sub_kodefikasi_tiket < 23 || $id_sub_sub_kodefikasi_tiket > 27) {
        header("Location: tiket.php");
        exit();
    } elseif ($id_sub_sub_kodefikasi_tiket === 26) {
        header("Location: faq-mahasiswa.php?id_sub_sub_kodefikasi_tiket=" . $id_sub_sub_kodefikasi_tiket);
        exit();
    } 
} elseif ($user_role === 'Dosen') {
    if ($id_sub_sub_kodefikasi_tiket === 28 || $id_sub_sub_kodefikasi_tiket > 33) {
        header("Location: tiket.php");
        exit();
    } elseif ($id_sub_sub_kodefikasi_tiket === 29) {
