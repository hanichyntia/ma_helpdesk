<!doctype php>
<php lang="en">

    <head>
        
        <meta charset="utf-8" />
        <title>Beranda Helpdesk</title>
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <meta content="Premium Multipurpose Admin & Dashboard Template" name="description" />
        <meta content="Themesbrand" name="author" />
        <!-- App favicon -->
        <link rel="shortcut icon" href="assets/images/favicon.ico">

        <!-- Bootstrap Css -->
        <link href="assets/css/bootstrap.min.css" id="bootstrap-style" rel="stylesheet" type="text/css" />
        <!-- Icons Css -->
        <link href="assets/css/icons.min.css" rel="stylesheet" type="text/css" />
        <!-- App Css-->
        <link href="assets/css/app.min.css" id="app-style" rel="stylesheet" type="text/css" />

    </head>

    <body>
        <div id="layout-wrapper">
        <header id="page-topbar">
                <div class="navbar-header">
                    <div class="d-flex">
                        <div class="navbar-brand-box">
                        <a href="" class="logo logo-ma-light">
                            <span class="logo-sm">
                                <img src="assets/images/logo-ma-warna-kecil.png" alt="Logo Small Dark" height="22">
                            </span>
                            <span class="logo-lg">
                                <img src="assets/images/logo-ma-warna.png" alt="Logo Large Dark" height="100">
                            </span>
                        </a>
                        </div>
                    </div>

                    <div class="" >
                        <a href="login-hlp.php" class="fw-medium text-primary">Butuh bantuan?</a>
                    </div>
                    
                </div>
            </header>
            <div class="main-content">

                <div class="page-content">
                    <div class="container-fluid">

                        
                        <!-- end page title -->

                        <div class="row">
                            <div class="col-xl-9 col-lg-8">
                                <div class="card">
                                    <div class="card-body">
                                        <div class="pt-3">
                                            <div class="row justify-content-center">
                                                <div class="col-xl-8">
                                                    <div>
                                                        <div class="text-center">
                                                            
                                                            <h4>Melihat KRS mata kuliah</h4>
                                                            
                                                        </div>
                                                        
                                                        <hr>

                                                        <div class="mt-4">
                                                            <div class="font-size-14">
                                                                <p>Mahasiswa dapat melihat KRS mata kuliah dengan membuka menu <strong>KRS → Lihat KRS</strong></p>

                                                                <img src="macismasis/melihat-krs-mata-kuliah-satu.png" alt="" class="img-thumbnail mx-auto d-block">
                                                                
                                                                <br>

                                                                <p class="text-muted mb-4 text-center">Tampilan menu MAC IS mahasiswa</p>

                                                                <p class="mb-4">Kemudian mahasiswa dapat melihat informasi mahasiswa yang terdiri dari:</p>

                                                                <p class="mb-4">1. Nama</p>

                                                                <p class="mb-4">2. Dosen PA</p>

                                                                <p class="mb-4">3. Program Studi</p>

                                                                <p class="mb-4">4. IPK terakhir</p>

                                                                <p class="mb-4">5. Maksimum pengambilan SKS</p>

                                                                <p class="mb-4">6. Pengambilan SKS saat ini</p>

                                                                <p class="mb-4">7. Tahun akademik dan semester</p>

                                                                <p class="mb-4">8. Status KRS</p>
                                                                
                                                                <p class="mb-4">Beserta mahasiswa juga dapat melihat informasi rekapitulasi poin keaktifan yang telah diperoleh yang terdiri dari:</p>

                                                                <p class="mb-4">1. Kategori penilaian atau rumpun poin keaktifan</p>
                                                                
                                                                <div style="margin-left: 20px;">
                                                                
                                                                <p class="mb-4">    a. Pengembangan diri</p>

                                                                <p class="mb-4">    b. Kepedulian sosial</p>

                                                                <p class="mb-4">    c. Penalaran dan keilmuan</p>

                                                                <p class="mb-4">    d. Bakat dan minat</p>

                                                                <p class="mb-4">    e. Kepempimpinan</p>

                                                                <p class="mb-4">    f. Wawasan almamater</p>
                                                                
                                                                </div>

                                                                <p class="mb-4">2. Besaran poin keaktifan yang telah diperoleh setiap rumpun</p>

                                                                <p class="mb-4">3. Besaran standar minimum poin keaktifan yang harus dicukupkan</p>

                                                                <img src="macismasis/melihat-krs-mata-kuliah-dua.png" alt="" class="img-thumbnail mx-auto d-block">
                                                                
                                                                <br>

                                                                <p class="text-muted mb-4 text-center">Tampilan menu lihat KRS pada bagian informasi mahasiswa dan rekapitulasi poin keaktifan</p>

                                                                <br>

                                                                <p class="mb-4">Mahasiswa juga dapat melihat informasi KRS mata kuliah yang telah diambil dengan terdiri dari:</p>

                                                                <p class="mb-4">1. Kode mata kuliah</p>

                                                                <p class="mb-4">2. Nama mata kuliah </p>

                                                                <p class="mb-4">3. Besaran SKS</p>

                                                                <p class="mb-4">4. Kode kelas</p>

                                                                <p class="mb-4">5. Nama ruangan</p>

                                                                <p class="mb-4">6. Hari</p>

                                                                <p class="mb-4">7. Jam mulai</p>

                                                                <p class="mb-4">8. Jam selesai</p>

                                                                <p class="mb-4">9. Status keikutsertaan mata kuliah</p>

                                                                <p class="mb-4">10. Button untuk mengubah status mata kuliah</p>
                                                                
                                                                <p class="mb-4">11. Opsi sebagai informasi mata kuliah tersebut sudah tervalidasi atau belum</p>

                                                                <p class="mb-4">12. Search bard untuk mencari mata kuliah</p>

                                                                <br>

                                                                <img src="macismasis/melihat-krs-mata-kuliah-tiga.png" alt="" class="img-thumbnail mx-auto d-block">

                                                                <br>

                                                                <p class="text-muted mb-4 text-center">Tampilan daftar KRS mata kuliah yang diambil</p>

                                                                <br>

                                                                <div class="text-center">
                                                                    <iframe width="560" height="315" src="https://youtube.com/embed/pZSGhdnjXtM" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                                                                </div>

                                                                <br>

                                                        </div>

                                                        <hr>
                                                        
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <!-- end card body -->
                                </div>
                                <!-- end card -->
                            </div>
                            <!-- end col -->
                        </div>
                        <!-- end row -->
                        
                    </div> <!-- container-fluid -->
                </div>
                <!-- End Page-content -->

                
                <footer class="footer">
                    <div class="container-fluid">
                        <div class="row">
                            <div class="col-sm-6">
                                <script>document.write(new Date().getFullYear())</script> © Skote.
                            </div>
                            <div class="col-sm-6">
                                <div class="text-sm-end d-none d-sm-block">
                                    Design & Develop by Themesbrand
                                </div>
                            </div>
                        </div>
                    </div>
                </footer>
            </div>
            <!-- end main content-->

        </div>
        <!-- END layout-wrapper -->

        <!-- Right bar overlay-->
        <div class="rightbar-overlay"></div>

        <!-- JAVASCRIPT -->
        <script src="assets/libs/jquery/jquery.min.js"></script>
        <script src="assets/libs/bootstrap/js/bootstrap.bundle.min.js"></script>
        <script src="assets/libs/metismenu/metisMenu.min.js"></script>
        <script src="assets/libs/simplebar/simplebar.min.js"></script>
        <script src="assets/libs/node-waves/waves.min.js"></script>
        <script src="assets/js/app.js"></script>

    </body>
</html>
